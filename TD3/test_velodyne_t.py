...
def _extract_world_rect_obstacles(world_path):
    obstacles = []
...
def _plot_episode_path(traj, starts, goal, world_path, output_path):
    obstacles = _extract_world_rect_obstacles(world_path)
    ...
    ax.set_xlabel("X / m")
    ax.set_ylabel("Y / m")
    ax.set_title("Three-robot path planning result")
    ...
    for obs in obstacles:
        ...
        rect = patches.Rectangle(
            ...
            obs["width"],
            obs["height"],
            facecolor="#8B5A2B" if is_fence else "#BDBDBD",
            edgecolor="#5D4037" if is_fence else "#424242",
            alpha=0.35 if is_fence else 0.75,
            linewidth=1.0,
            label="obstacle" if name != "fence" else None,
        )
        rect.set_transform(
    ...
    for i, points in enumerate(traj):
        ...
        if len(arr) > 0:
            ax.plot(arr[:, 0], arr[:, 1], color=colors[i], linewidth=2.0, label=labels[i])
        ax.scatter(starts[i][0], starts[i][1], color=colors[i], marker="o", s=70, edgecolor="black", zorder=5)
        ax.text(starts[i][0] + 0.08, starts[i][1] + 0.08, f"S{i}", color=colors[i], weight="bold")

    ax.scatter(goal[0], goal[1], color="gold", marker="*", s=180, edgecolor="black", zorder=6, label="leader goal")
...
class RelativeFormationFollower:
    ...
    def act(self, leader_pose, follower_pose, offset_body_xy, leader_cmd, laser_ranges=None):
        ...
        if avoid["emergency"]:
            ...
            v_cmd *= avoid_scale
            w_cmd = 0.50 * w_cmd + 0.50 *avoid_turn
        else:
            v_cmd *= avoid_scale
...
fixed_goals = [(4, 3.0), (3.0, 4.0), (2.6, 4.0)]
fixed_leader_start = (-3.0, -2.0)
# 自定义领航车起始朝向，单位为弧度；三台车初始朝向都会使用该值。
# 例如：0.0 朝 +X，math.pi / 2 朝 +Y，-math.pi / 2 朝 -Y。
fixed_leader_start_yaw = math.pi / 2
LEADER_V_MAX = 0.5
follower_offsets = [(-0.8, 0), (-1.6, 0)]
DEBUG_FOLLOWERS = True
DEBUG_PRINT_EVERY = 10
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parent
WORLD_PATH = PROJECT_ROOT / "catkin_ws" / "src" / "multi_robot_scenario" / "launch" / "TD31.world"
PLOT_OUTPUT_PATH = SCRIPT_DIR / "results" / "TD31_episode_path.png"

def _formation_fixed_starts(leader_start, leader_yaw, offsets):
...
fixed_starts = _formation_fixed_starts(fixed_leader_start, fixed_leader_start_yaw, follower_offsets)
fixed_yaws = [fixed_leader_start_yaw for _ in range(num_agents)]
...
env = GazeboEnv(
    ...
    fixed_yaws=fixed_yaws if use_fixed_start_goal else None,
)
